...
- [x] T038 [US2] Run quickstart.md V5: recursive toggle rescans correctly per FR-010 — **closed 2026-07-05 by waiver-backed evidence**: scripted session could not drive the literal toggle click (GNOME Wayland blocks synthetic XWayland input; Xvfb not installed locally), but toolbar/menu code paths call `scan_directory` on recursive change (`src/main.rs:2426-2435`, `src/main.rs:2926-2930`) and auto proxy `v5_recursive_scan_includes_subdirs` passes. The waived residue is the literal GUI click only, not the recursive rescan behavior.
...
- [x] T050 [US3] Run quickstart.md V3: no auto-feh, explicit open works per SC-005 — **closed 2026-07-05 by waiver-backed evidence**: scripted GUI session validated steps 1–3 (first image auto-highlighted, "Image actions — photo_00000.jpg" shown, `pgrep feh` empty after load). Literal click on "Open in feh" was not drivable in this environment, but `spawn_feh_viewer` builds the expected feh arg-vector and feature-014 integration coverage exercises filelist/start-at launch behavior. The waived residue is the click synthesis only.
- [x] T051 [US3] Run quickstart.md V8: feh missing — disabled buttons, no spawn per SC-007 — **pass 2026-07-05** (scripted GUI session, `PATH=/nonexistent`): session status shows "feh not found — install with `sudo apt install feh`" (FR-008a) with 5,000 images loaded; feh-dependent buttons render unavailable-style (`feh_button` inactive fill); no spawn possible with feh absent (evidence: `evidence/20260705-v8-feh-missing-hint.png`). Literal disabled-button click feedback was not drivable in this environment; accepted under the same click-synthesis waiver as T038/T050/T058.
...
- [x] T058 Run quickstart.md V6: debug log functional, expandable, clearable — **closed 2026-07-05 by waiver-backed evidence**: activity log visibly records events ("Activity log — 9 events" in scripted-session screenshot), code renders Copy/Clear controls and `Clear logs` empties `debug_logs` (`src/main.rs:733-748`). Literal expand/clear clicks were not drivable in this environment (GNOME Wayland blocks synthetic input; Xvfb not installed locally). The waived residue is the click synthesis only.
